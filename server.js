...
app.get("/nodejs-eventloop.pdf", async (req, res) => {
  let browser;
  try {
    browser = await puppeteer.launch({
      headless: "new",
    ...
    const page = await browser.newPage();
    const fileUrl = `http://localhost:${PORT}/nodejs-eventloop-questions.html`;
    ...
    const pdfBuffer = await page.pdf({
      ...
      headerTemplate: "<span></span>",
      footerTemplate: `
        <div style="width:100%; font-size:9px; color:#e68815; padding:0 26px; padding-bottom: 5px; box-sizing:border-box; display:flex; justify-content:space-between; align-items:center;">
          <div style="display:flex; align-items:center; gap:6px;">
            <svg width="10" height="10" viewBox="0 0 24 24" fill="#0a66c2" xmlns="http://www.w3.org/2000/svg">
              <path d="M4.98 3.5C4.98 4.88 3.88 6 2.5 6S0 4.88 0 3.5 1.12 1 2.5 1 4.98 2.12 4.98 3.5zM0.2 8h4.6v15H0.2V8zM7.7 8h4.4v2.1h.1c.6-1.1 2.1-2.3 4.4-2.3 4.7 0 5.6 3.1 5.6 7.1V23h-4.6v-6.7c0-1.6 0-3.7-2.2-3.7-2.2 0-2.6 1.8-2.6 3.6V23H7.7V8z"/>
            </svg>
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=\"NodeJS_Event_Loop_Questions.pdf\"",
    );
    res.send(pdfBuffer);
  } catch (error) {
    console.error("PDF Generation Error:", error);
...
app.listen(PORT, () => {
  ...
  console.log(
    `Download Node.js Event Loop questions at http://localhost:${PORT}/nodejs-eventloop.pdf`,
  );
});
